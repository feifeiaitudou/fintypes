...
func (a Asset) Type() AssetType {
	s := string(a)
	if len(s) > 2 {
		if stringz.StartWith(s, "f.") {
			return AssetTypeFiat
		} else if stringz.StartWith(s, "b.") {
			return AssetTypeCoin
		} else if stringz.StartWith(s, "c.") {
			return AssetTypeCoin
		} else if stringz.StartWith(s, "m.") {
			return AssetTypeMetal
		} else if stringz.StartWith(s, "s.") {
			return AssetTypeStock
		} else if stringz.StartWith(s, "i.") {
...
func (a Asset) ToCoin() (platform Platform, name, symbol string, ok bool) {
	if a.Type() == AssetTypeCoin {
		if ss := strings.Split(string(a)[2:], "@"); len(ss) == 2 {
			...
			if stringz.StartWith(string(a), "b.") {
				if plt, err := ParsePlatform(ss[1]); err == nil {
					return plt, "", ss[0], true
				}
			}

			if stringz.StartWith(string(a), "c.") {
				if plt, err := ParsePlatform(ss[1]); err == nil {
...
func NewCoin(name, symbol string, plt Platform) Asset {
	...
	return Asset("c." + stringz.OnlyFirstLetterUpperCase(name) + "." + strings.ToUpper(symbol) + "@" + strings.ToLower(plt.String()))
}
...
func NewAsset(sdn string) (Asset, error) {
	...
	if stringz.StartWith(sdn, "f.") {
		return newFiat(sdn[2:])
	} else if stringz.StartWith(sdn, "b.") {
	...
	} else if stringz.StartWith(sdn, "c.") {
		if ss := strings.Split(sdn[2:], "@"); len(ss) == 2 {
			plt, err := ParsePlatform(ss[1])
			if err != nil {
				return AssetNil, err
	...
	} else if stringz.StartWith(sdn, "m.") {
		return newMetal(sdn[2:]), nil
	} else if stringz.StartWith(sdn, "s.") {
	...
	} else if stringz.StartWith(sdn, "i.") {

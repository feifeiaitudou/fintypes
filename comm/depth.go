...
type (
	OrderBook struct {
		Price  decimals.Decimal `json:"Price"`
		Amount decimals.Decimal `json:"Amount"`
	}
	...
	DepthTolerance struct {
		UnitDealAmount  decimals.Decimal
		QuoteDealAmount decimals.Decimal
		Price1          decimals.Decimal
		PriceReach      decimals.Decimal // 上/下探价格, -1 means 击穿
	}
)

var (
	PricePiercing = decimals.NewFromInt(-1)
)
...
// 获取挂单中币的总量
func GetTotalAmount(input []OrderBook) decimals.Decimal {
	r := decimals.Zero
	for _, v := range input {
		r = r.Add(v.Amount)
	}
...
// 获取挂单中（如果都成交）基准货币的总量
func GetTotalPrice(input []OrderBook) decimals.Decimal {
	r := decimals.Zero
	for _, v := range input {
...
func (drd *DepthRawData) GetMinSell() (price, amount decimals.Decimal, err error) {
	if drd.Sells.Len() == 0 {
		return decimals.Zero, decimals.Zero, errors.Errorf("Empty sells")
	}
	cpy := drd.Sells
	sort.Sort(cpy)
...
func (drd *DepthRawData) GetMaxBuy() (price, amount decimals.Decimal, err error) {
	if drd.Buys.Len() == 0 {
		return decimals.Zero, decimals.Zero, errors.Errorf("Empty sells")
	}
	cpy := drd.Buys
...
func (d *Depth) ParseSellOnePrice() decimals.Decimal {
	return d.Sells[0].Price
}

func (d *Depth) ParseBuyOnePrice() decimals.Decimal {
	return d.Buys[0].Price
}

func (d *Depth) ParseBuyerTotal() (avgPrice, totalAmount decimals.Decimal) {
	totalPrice := decimals.Zero
...
func (d *Depth) ParseSellerTotal() (avgPrice, totalAmount decimals.Decimal) {
	totalPrice := decimals.Zero
...
// 砸盘探测，能砸多深
func (d *Depth) MarketSellDetectEx(unitAmount, slippageAllowed decimals.Decimal) DepthTolerance {
	r := DepthTolerance{}
	if d.DepthRawData.Buys.Len() > 0 {
		r.Price1 = d.DepthRawData.Buys[0].Price
	}
	r.PriceReach = PricePiercing
	minPriceAllowed := r.Price1.Mul(decimals.One.Sub(slippageAllowed)) // 最小忍受价格
	...
	for _, orderBook := range d.DepthRawData.Buys {
		...
		dealUnit := decimals.Min(orderBook.Amount, leftUnit)
		...
		if leftUnit.LessThanOrEqual(decimals.Zero) {
			break
		}
	}
	return r
...
func (d *Depth) MarketBuyDetectEx(quoteAmount decimals.Decimal, slippageAllowed decimals.Decimal, precision int, lot decimals.Decimal) DepthTolerance {
	r := DepthTolerance{}
	...
	r.PriceReach = PricePiercing
	maxPriceAllowed := r.Price1.Mul(decimals.One.Add(slippageAllowed)) // 最大忍受价格
	...
	for _, orderBook := range d.DepthRawData.Sells {
		...
		dealQuote := decimals.Min(orderBook.Amount.Mul(orderBook.Price), leftQuote)
		...
		if leftQuote.LessThanOrEqual(decimals.Zero) {
			break
		}
	}
...
// 砸盘探测，能砸多深
func (d *Depth) MarketSellDetect(unitAmount decimals.Decimal) DepthTolerance {
	r := DepthTolerance{}
	if d.DepthRawData.Buys.Len() > 0 {
		r.Price1 = d.DepthRawData.Buys[0].Price
	}
...
// 拉盘探测，能拉多高
func (d *Depth) MarketBuyDetect(quoteAmount decimals.Decimal) DepthTolerance {
	r := DepthTolerance{}

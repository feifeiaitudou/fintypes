...
func (p Period) ToDuration() time.Duration {
	sec := p.ToSeconds()
	return clock.MulDuration(sec, time.Second)
}
...
func (p Period) ToDurationExact(t time.Time, tz *time.Location) time.Duration {
	...
	if p == Period1MonthFUZZY {
		return clock.GetMonthDuration(t.In(tz).Year(), int(t.In(tz).Month()))
	} else if p == Period1YearFUZZY {
		return clock.GetYearDuration(t.In(tz).Year())
	} else {
		sec := p.ToSeconds()
		return clock.MulDuration(sec, time.Second)
	}
...
func (p Period) CustomFormat(config *ExConfig) (string, error) {
...
var (
	DefaultPeriodRoundConfig PeriodRoundConfig = PeriodRoundConfig{WeekBegin: time.Sunday, UseLocalZeroOClockAsDayBeginning: false}
)
...
func RoundPeriodEarlier(dotTime time.Time, period Period, prc PeriodRoundConfig) time.Time {
	...
	if period == Period1YearFUZZY {
		...
		return clock.Sub(dotTime, clock.Day*time.Duration(days))
		...
		return clock.RoundEarlier(dotTime, period.ToDuration()).In(tz)
	}
}
